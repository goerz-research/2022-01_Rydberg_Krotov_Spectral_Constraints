...
function _convert_freq_to_float(s::String)
    if endswith(s, "GHz")
        ...
        @warn("Value $s taken to be in internal units. Consider '$(s)GHz' instead")
        return parse(s)
    end
end
